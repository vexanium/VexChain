...
BOOST_AUTO_TEST_CASE_TEMPLATE(light_validation_skip_cfa, TYPE_T, backing_store_type_suite) { try {
   ...
   cfg.trusted_producers = { "vexcore"_n }; // light validation
...
BOOST_AUTO_TEST_CASE_TEMPLATE(deferred_transaction_tests, TESTER_T, backing_store_ts) { try {
...

   {
      ...
      // fail because testapi@additional permission is not unilaterally satisfied by testapi@vex.code.
      dtt_action dtt_act3;
...
// This WASM expects that the current producers are the default {"vexcore"}.
...
BOOST_AUTO_TEST_CASE_TEMPLATE(resource_limits_tests, TESTER_T, backing_store_ts) {
   ...
   t.push_action( "vexcore"_n, "setpriv"_n, "vexcore"_n, mutable_variant_object()("account", "rlimits"_n)("is_priv", 1));
   t.produce_block();
