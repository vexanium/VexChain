...
public:
   ...
   eosio_token_tester() {
      ...
      create_accounts( { "alice"_n, "bob"_n, "carol"_n, "vex.token"_n } );
      produce_blocks( 2 );
   ...
   fc::variant get_account( account_name acc, const string& symbolname)
   {
      auto symb = eosio::chain::symbol::from_string(symbolname);
      auto symbol_code = symb.to_symbol_code().value;
      vector<char> data = get_row_by_account( "vex.token"_n, acc, "accounts"_n, name(symbol_code) );
   ...
   action_result create( account_name issuer,
      ...
      return push_action( "vex.token"_n, "create"_n, mvo()
           ( "issuer", issuer)
