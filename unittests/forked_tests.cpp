...
BOOST_AUTO_TEST_CASE( forking ) try {
   ...
   auto r2 = c.create_accounts( {"vex.token"_n} );
   wdump((fc::json::to_pretty_string(r2)));
   c.set_code( "vex.token"_n, contracts::eosio_token_wasm() );
   c.set_abi( "vex.token"_n, contracts::eosio_token_abi().data() );
   c.produce_blocks(10);


   auto cr = c.push_action( "vex.token"_n, "create"_n, "vex.token"_n, mutable_variant_object()
              ("issuer",       "vexcore" )
              ("maximum_supply", core_from_string("10000000.0000"))
      );

   cr = c.push_action( "vex.token"_n, "issue"_n, config::system_account_name, mutable_variant_object()
              ("to",       "vexcore" )
              ("quantity", core_from_string("100.0000"))
              ("memo", "")
      );

   cr = c.push_action( "vex.token"_n, "transfer"_n, config::system_account_name, mutable_variant_object()
              ("from",     "vexcore")
              ("to",       "dan" )
              ("quantity", core_from_string("100.0000"))
              ("memo", "")
