...
class currency_tester : public TESTER {
   public:
      ...
      asset get_balance(const account_name& account) const {
         return get_currency_balance("vex.token"_n, symbol(SY(4,CUR)), account);
      }
      ...
      auto issue(const account_name& to, const std::string& quantity, const std::string& memo = "") {
         auto trace = push_action("vex.token"_n, "issue"_n, mutable_variant_object()
                                  ("to",       to)
                                  ("quantity", quantity)
                                  ("memo",     memo)
                                  );
      ...
      {
         create_account( "vex.token"_n);
         set_code( "vex.token"_n, contracts::eosio_token_wasm() );

         auto result = push_action("vex.token"_n, "create"_n, mutable_variant_object()
                 ("issuer",       eosio_token)
         ...
         result = push_action("vex.token"_n, "issue"_n, mutable_variant_object()
                 ("to",       eosio_token)
...
const name currency_tester::eosio_token = "vex.token"_n;
...
BOOST_AUTO_TEST_CASE( bootstrap ) try {
   ...
   currency_tester t;
   auto actual = t.get_currency_balance("vex.token"_n, expected.get_symbol(), "vex.token"_n);
...
BOOST_FIXTURE_TEST_CASE( test_transfer, currency_tester ) try {
   ...
   {
      auto trace = push_action("vex.token"_n, "transfer"_n, mutable_variant_object()
         ("from", eosio_token)
         ("to",   "alice")
...
BOOST_FIXTURE_TEST_CASE( test_duplicate_transfer, currency_tester ) {
   ...
   BOOST_REQUIRE_THROW(push_action("vex.token"_n, "transfer"_n, mutable_variant_object()
                                    ("from", eosio_token)
                                    ("to",   "alice")
                                    ("quantity", "100.0000 CUR")
                                    ("memo", "fund Alice")),
...
BOOST_FIXTURE_TEST_CASE(test_symbol, TESTER) try {
   ...
   {
      symbol sys(4, "VEX");
      BOOST_REQUIRE_EQUAL(SY(4,VEX), sys.value());
      BOOST_REQUIRE_EQUAL("4,VEX", sys.to_string());
      BOOST_REQUIRE_EQUAL("VEX", sys.name());
