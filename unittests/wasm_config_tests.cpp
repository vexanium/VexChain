...
struct wasm_config_tester : TESTER {
   ...
   void set_wasm_params(const wasm_config& params) {
      ...
      trx.actions.emplace_back(vector<permission_level>{{"vexcore"_n,config::active_name}}, "vexcore"_n, "setwparams"_n,
                               bios_abi_ser.variant_to_binary("setwparams", fc::mutable_variant_object()("cfg", params),
                                                              abi_serializer::create_yield_function( abi_serializer_max_time )));
      trx.actions[0].authorization = vector<permission_level>{{"vexcore"_n,config::active_name}};
      set_transaction_headers(trx);
      trx.sign(get_private_key("vexcore"_n, "active"), control->get_chain_id());
...
// that would prevent it from being set on the vexcore account and executing.
...
BOOST_FIXTURE_TEST_CASE(reset_chain_tests, wasm_config_tester) {
   ...
   // Reset parameters and system contract
   {
      ...
      auto make_setcode = [](const std::vector<uint8_t>& code) {
         return setcode{ "vexcore"_n, 0, 0, bytes(code.begin(), code.end()) };
      };
      trx.actions.push_back({ { { "vexcore"_n, config::active_name} }, make_setcode(wast_to_wasm(min_set_parameters_wast)) });
      trx.actions.push_back({ { { "vexcore"_n, config::active_name} }, "vexcore"_n, ""_n, fc::raw::pack(genesis_state::default_initial_wasm_configuration) });
      trx.actions.push_back({ { { "vexcore"_n, config::active_name} }, make_setcode(contracts::eosio_bios_wasm()) });
      set_transaction_headers(trx);
      trx.sign(get_private_key("vexcore"_n, "active"), control->get_chain_id());
      push_transaction(trx);
   }
