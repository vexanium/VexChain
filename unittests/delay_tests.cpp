...
asset get_currency_balance(const TESTER& chain, account_name account) {
   return chain.get_currency_balance("vex.token"_n, symbol(SY(4,CUR)), account);
}

const std::string eosio_token = name("vex.token"_n).to_string();
...
BOOST_AUTO_TEST_CASE( link_delay_direct_test ) { try {
   ...
   chain.create_account("vex.token"_n);
   chain.produce_blocks(10);

   chain.set_code("vex.token"_n, contracts::eosio_token_wasm());
   chain.set_abi("vex.token"_n, contracts::eosio_token_abi().data());
   ...
   chain.push_action(config::system_account_name, linkauth::get_name(), tester_account, fc::mutable_variant_object()
           ...
           ("requirement", "first"));
   chain.produce_blocks();
   chain.push_action("vex.token"_n, "create"_n, "vex.token"_n, mutable_variant_object()
           ("issuer", eosio_token)
           ("maximum_supply", "9000000.0000 CUR")
   );


   chain.push_action("vex.token"_n, name("issue"), "vex.token"_n, fc::mutable_variant_object()
           ("to",       eosio_token)
           ("quantity", "1000000.0000 CUR")
           ("memo", "for stuff")
   );

   auto trace = chain.push_action("vex.token"_n, name("transfer"), "vex.token"_n, fc::mutable_variant_object()
       ("from", eosio_token)
       ("to", "tester")
   ...
   auto liquid_balance = get_currency_balance(chain, "vex.token"_n);
   ...
   trace = chain.push_action("vex.token"_n, name("transfer"), "tester"_n, fc::mutable_variant_object()
       ("from", "tester")
       ("to", "tester2")
       ("quantity", "1.0000 CUR")
       ("memo", "hi" )
   ...
   liquid_balance = get_currency_balance(chain, "vex.token"_n);
   BOOST_REQUIRE_EQUAL(asset::from_string("999900.0000 CUR"), liquid_balance);
   liquid_balance = get_currency_balance(chain, "tester"_n);
...
BOOST_AUTO_TEST_CASE(delete_auth_test) { try {
   ...
   // transfer from vex.token to tester
   trace = chain.push_action("vex.token"_n, name("transfer"), "vex.token"_n, fc::mutable_variant_object()
       ("from", "vex.token")
       ("to", "tester")
       ("quantity", "100.0000 CUR")
       ("memo", "hi" )
...
BOOST_AUTO_TEST_CASE( mindelay_test ) { try {
   ...
   const auto& acnt = chain.control->db().get<account_object,by_name>("vex.token"_n);
   ...
   const auto a = chain.control->db().get<account_object,by_name>("vex.token"_n).get_abi();

   string action_type_name = abis.get_action_type(name("transfer"));

   action act;
   act.account = "vex.token"_n;
...
BOOST_AUTO_TEST_CASE( canceldelay_test2 ) { try {
...

   {
      ...
      trace = chain.push_action("vex.token"_n, name("transfer"), vector<permission_level>{{"tester"_n, "first"_n}}, fc::mutable_variant_object()
          ("from", "tester")
          ("to", "tester2")
          ("quantity", "1.0000 CUR")
          ("memo", "hi" ),
   ...
   {
      // this transaction will be delayed 10 blocks
      trace = chain.push_action("vex.token"_n, name("transfer"), vector<permission_level>{{"tester"_n, "second"_n}}, fc::mutable_variant_object()
          ("from", "tester")
          ("to", "tester2")
          ("quantity", "5.0000 CUR")
          ("memo", "hi" ),
   ...
   {
      // this transaction will be delayed 10 blocks
      trace = chain.push_action("vex.token"_n, name("transfer"), vector<permission_level>{{"tester"_n, config::owner_name}}, fc::mutable_variant_object()
          ("from", "tester")
          ("to", "tester2")
          ("quantity", "10.0000 CUR")
          ("memo", "hi" ),
