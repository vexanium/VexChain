...
BOOST_AUTO_TEST_CASE( fix_linkauth_restriction ) { try {
   ...
   validate_disallow("vexcore", "linkauth");
   validate_disallow("vexcore", "unlinkauth");
   validate_disallow("vexcore", "deleteauth");
   validate_disallow("vexcore", "updateauth");
   validate_disallow("vexcore", "canceldelay");
...
BOOST_AUTO_TEST_CASE( forward_setcode_test ) { try {
   ...
   //   * eosio::setcode to set code on the vexcore is allowed (unless the rejectall account exists)
   ...
   // Activate FORWARD_SETCODE protocol feature and then return contract on vexcore back to what it was.
   const auto& pfm = c.control->get_protocol_feature_manager();
   ...
   // It will now not be possible to deploy the reject_all contract to the vexcore account,
   ...
   c2.produce_block();
   // Now all actions dispatched to the vexcore account should be rejected.

   // However, it should still be possible to set the bios contract because the WASM on vexcore is called after the
...
BOOST_AUTO_TEST_CASE( producer_schedule_change_extension_test ) { try {
   ...
   { // ensure producer_schedule_change_extension is rejected
      ...
      bad_block->producer_signature = remote.get_private_key("vexcore"_n, "active").sign(sig_digest);
...
BOOST_AUTO_TEST_CASE( wtmsig_block_signing_inflight_legacy_test ) { try {
   ...
   c.preactivate_protocol_features( {*d} );
   vector<legacy::producer_key> sched = {{"vexcore"_n, c.get_public_key("vexcore"_n, "bsk")}};
   ...
   c.block_signing_private_keys.emplace(get_public_key("vexcore"_n, "bsk"), get_private_key("vexcore"_n, "bsk"));
   c.produce_block();
