...
class whitelist_blacklist_tester {
   public:
      ...
      void init( bool bootstrap = true ) {
         ...
         chain->create_accounts({"vex.token"_n, "alice"_n, "bob"_n, "charlie"_n});
         chain->set_code("vex.token"_n, contracts::eosio_token_wasm() );
         chain->set_abi("vex.token"_n, contracts::eosio_token_abi().data() );
         chain->push_action( "vex.token"_n, "create"_n, "vex.token"_n, mvo()
              ( "issuer", "vex.token" )
              ( "maximum_supply", "1000000.00 TOK" )
         );
         chain->push_action( "vex.token"_n, "issue"_n, "vex.token"_n, mvo()
              ( "to", "vex.token" )
      ...
      transaction_trace_ptr transfer( account_name from, account_name to, string quantity = "1.00 TOK" ) {
         return chain->push_action( "vex.token"_n, "transfer"_n, from, mvo()
            ( "from", from )
            ( "to", to )
            ( "quantity", quantity )
            ( "memo", "" )
...
BOOST_AUTO_TEST_CASE( actor_whitelist ) { try {
   ...
   test.actor_whitelist = {config::system_account_name, "vex.token"_n, "alice"_n};
   test.init();

   test.transfer( "vex.token"_n, "alice"_n, "1000.00 TOK" );
...
BOOST_AUTO_TEST_CASE( contract_whitelist ) { try {
   whitelist_blacklist_tester<> test;
   test.contract_whitelist = {config::system_account_name, "vex.token"_n, "bob"_n};
   test.init();

   test.transfer( "vex.token"_n, "alice"_n, "1000.00 TOK" );

   test.transfer( "alice"_n, "vex.token"_n );
...
BOOST_AUTO_TEST_CASE( action_blacklist ) { try {
   whitelist_blacklist_tester<> test;
   test.contract_whitelist = {config::system_account_name, "vex.token"_n, "bob"_n, "charlie"_n};
...
BOOST_AUTO_TEST_CASE( actor_blacklist_inline_deferred ) { try {
   ...
   tester1.chain->push_action( "vexcore"_n, "updateauth"_n, "alice"_n, mvo()
      ( "account", "alice" )
      ( "permission", "active" )
      ( "parent", "owner" )
      ( "auth", auth )
   ...
   tester1.chain->push_action( "vexcore"_n, "updateauth"_n, "bob"_n, mvo()
      ( "account", "bob" )
      ( "permission", "active" )
      ( "parent", "owner" )
      ( "auth", auth )
   ...
   tester1.chain->push_action( "vexcore"_n, "updateauth"_n, "charlie"_n, mvo()
      ( "account", "charlie" )
      ( "permission", "active" )
      ( "parent", "owner" )
      ( "auth", auth )
   ...
   auto log_trxs = [&](std::tuple<const transaction_trace_ptr&, const packed_transaction_ptr&> x) {
      ...
      if( act.account == "vexcore"_n && act.name == "onblock"_n ) return;
