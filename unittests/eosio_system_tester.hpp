...
public:
   ...
   eosio_system_tester(Lambda setup) {
      ...
      create_accounts({ "vex.token"_n, "vex.ram"_n, "vex.ramfee"_n, "vex.stake"_n,
               "vex.bpay"_n, "vex.vpay"_n, "vex.saving"_n, "vex.names"_n });

      produce_blocks( 100 );

      set_code( "vex.token"_n, contracts::eosio_token_wasm() );
      set_abi( "vex.token"_n, contracts::eosio_token_abi().data() );

      {
         const auto& accnt = control->db().get<account_object,by_name>( "vex.token"_n );
         abi_def abi;
         BOOST_REQUIRE_EQUAL(abi_serializer::to_abi(accnt.abi, abi), true);
         token_abi_ser.set_abi(abi, abi_serializer::create_yield_function( abi_serializer_max_time ));
      }

      create_currency( "vex.token"_n, config::system_account_name, core_from_string("10000000000.0000") );
      issue(config::system_account_name,      core_from_string("1000000000.0000"));
      BOOST_REQUIRE_EQUAL( core_from_string("1000000000.0000"), get_balance( name("vexcore") ) );
      ...
      BOOST_REQUIRE_EQUAL( core_from_string("1000000000.0000"),
            get_balance(name("vexcore")) + get_balance(name("vex.ramfee")) + get_balance(name("vex.stake")) + get_balance(name("vex.ram")) );
   }
   ...
   asset get_balance( const account_name& act ) {
      vector<char> data = get_row_by_account( "vex.token"_n, act, "accounts"_n, name(symbol(CORE_SYMBOL).to_symbol_code().value) );
   ...
   void issue( name to, const asset& amount, name manager = config::system_account_name ) {
      base_tester::push_action( "vex.token"_n, "issue"_n, manager, mutable_variant_object()
                                ("to",      to )
                                ("quantity", amount )
                                ("memo", "")
                                );
   ...
   void transfer( name from, name to, const asset& amount, name manager = config::system_account_name ) {
      base_tester::push_action( "vex.token"_n, "transfer"_n, manager, mutable_variant_object()
                                ("from",    from)
                                ("to",      to )
                                ("quantity", amount)
                                ("memo", "")
   ...
   fc::variant get_stats( const string& symbolname ) {
      ...
      vector<char> data = get_row_by_account( "vex.token"_n, name(symbol_code), "stat"_n, name(symbol_code) );
   ...
   abi_serializer initialize_multisig() {
      ...
      {
         create_account_with_resources( "vex.msig"_n, config::system_account_name );
         BOOST_REQUIRE_EQUAL( success(), buyram( name("vexcore"), name("vex.msig"), core_from_string("5000.0000") ) );
         produce_block();
         ...
         set_code( "vex.msig"_n, contracts::eosio_msig_wasm() );
         set_abi( "vex.msig"_n, contracts::eosio_msig_abi().data() );

         produce_blocks();
         const auto& accnt = control->db().get<account_object,by_name>( "vex.msig"_n );
         abi_def msig_abi;
   ...
   vector<name> active_and_vote_producers() {
      ...
      transfer( name("vexcore"), name("alice1111111"), core_from_string("650000000.0000"), name("vexcore") );
