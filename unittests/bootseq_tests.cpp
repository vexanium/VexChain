...
public:
   ...
   fc::variant get_global_state() {
    ...
    asset get_balance( const account_name& act ) {
         return get_currency_balance("vex.token"_n, symbol(CORE_SYMBOL), act);
    }
...
BOOST_FIXTURE_TEST_CASE( bootseq_test, bootseq_tester ) {
    try {
        ...
        // Create vex.msig and vex.token
        create_accounts({"vex.msig"_n, "vex.token"_n, "vex.ram"_n, "vex.ramfee"_n, "vex.stake"_n, "vex.vpay"_n, "vex.bpay"_n, "vex.saving"_n });
        // Set code for the following accounts:
        //  - vexcore(code: eosio.bios) (already set by tester constructor)
        //  - vex.msig (code: vex.msig)
        //  - vex.token (code: vex.token)
        // set_code_abi("vex.msig"_n, contracts::eosio_msig_wasm(), contracts::eosio_msig_abi().data());//, &eosio_active_pk);
        // set_code_abi("vex.token"_n, contracts::eosio_token_wasm(), contracts::eosio_token_abi().data()); //, &eosio_active_pk);

        set_code_abi("vex.msig"_n,
                     contracts::eosio_msig_wasm(),
                     contracts::eosio_msig_abi().data());//, &eosio_active_pk);
        set_code_abi("vex.token"_n,
                     contracts::eosio_token_wasm(),
                     contracts::eosio_token_abi().data()); //, &eosio_active_pk);

        // Set privileged for vex.msig and vex.token
        set_privileged("vex.msig"_n);
        set_privileged("vex.token"_n);

        // Verify vex.msig and vex.token is privileged
        const auto& eosio_msig_acc = get<account_metadata_object, by_name>("vex.msig"_n);
        BOOST_TEST(eosio_msig_acc.is_privileged() == true);
        const auto& eosio_token_acc = get<account_metadata_object, by_name>("vex.token"_n);
        BOOST_TEST(eosio_token_acc.is_privileged() == true);


        // Create VEX tokens in vex.token, set its manager as eosio
        ...
        create_currency("vex.token"_n, config::system_account_name, max_supply);
        // Issue the genesis supply of 1 billion VEX tokens to eosio.system
        issue("vex.token"_n, config::system_account_name, config::system_account_name, initial_supply);
        ...
        for( const auto& a : test_genesis ) {
           ...
           r = delegate_bandwidth("vex.stake"_n, a.aname, asset(net), asset(cpu));
           BOOST_REQUIRE( !r->except_ptr );
        }
        ...
        BOOST_TEST(active_schedule.producers.front().producer_name == name("vexcore"));
