...
BOOST_AUTO_TEST_CASE(test_deltas_permission_link) {
   for (backing_store_type backing_store : { backing_store_type::CHAINBASE, backing_store_type::ROCKSDB }) {
      ...
      chain.link_authority("newacc"_n, "vexcore"_n, "spending"_n, "reqauth"_n);
...
BOOST_AUTO_TEST_CASE(test_deltas_kv) {
   for (backing_store_type backing_store : { backing_store_type::CHAINBASE, backing_store_type::ROCKSDB }) {
      ...
      chain.push_action("vexcore"_n, "ramkvlimits"_n, "vexcore"_n, mutable_variant_object()("k", 1024)("v", 1024)("i", 1024));
      chain.produce_block();
...
BOOST_AUTO_TEST_CASE(test_deltas_table_and_kv) {
   for (backing_store_type backing_store : { backing_store_type::CHAINBASE }) {
      ...
      auto trace = chain.push_action("vexcore"_n, "ramkvlimits"_n, "vexcore"_n, mutable_variant_object()("k", 1024)("v", 1024)("i", 1024));
      BOOST_REQUIRE_EQUAL(transaction_receipt::executed, trace->receipt->status);
...
BOOST_AUTO_TEST_CASE(test_deltas_resources_history) {
   for (backing_store_type backing_store : { backing_store_type::CHAINBASE, backing_store_type::ROCKSDB }) {
      ...
      chain.create_accounts({ "vex.token"_n, "vex.ram"_n, "vex.ramfee"_n, "vex.stake"_n});
      ...
      chain.push_action("vex.token"_n, "issue"_n, "vex.token"_n, fc::mutable_variant_object()
         ("to",       "vexcore")
