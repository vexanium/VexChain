...
issue_tokens( TESTER& t, account_name issuer, account_name to, const asset& amount,
              std::string memo = "", account_name token_contract = "vex.token"_n )
{
...
BOOST_AUTO_TEST_CASE_TEMPLATE( get_scope_test, TESTER_T, backing_store_ts) { try {
   ...
   t.create_accounts({ "vex.token"_n, "vex.ram"_n, "vex.ramfee"_n, "vex.stake"_n,
      "vex.bpay"_n, "vex.vpay"_n, "vex.saving"_n, "vex.names"_n });

   std::vector<account_name> accs{"inita"_n, "initb"_n, "initc"_n, "initd"_n};
   t.create_accounts(accs);
   t.produce_block();

   t.set_code( "vex.token"_n, contracts::eosio_token_wasm() );
   t.set_abi( "vex.token"_n, contracts::eosio_token_abi().data() );
   t.produce_blocks(1);

   // create currency
   auto act = mutable_variant_object()
         ("issuer",       "vexcore")
         ("maximum_supply", eosio::chain::asset::from_string("1000000000.0000 VEX"));
   t.push_action("vex.token"_n, "create"_n, "vex.token"_n, act );

   // issue
   for (account_name a: accs) {
      issue_tokens( t, config::system_account_name, a, eosio::chain::asset::from_string("999.0000 VEX") );
   }
   t.produce_blocks(1);

   // iterate over scope
   eosio::chain_apis::read_only plugin(*(t.control), {}, fc::microseconds::maximum());
   eosio::chain_apis::read_only::get_table_by_scope_params param{"vex.token"_n, "accounts"_n, "inita", "", 10};
   ...
   if (result.rows.size() >= 4) {
      BOOST_REQUIRE_EQUAL(name("vex.token"_n), result.rows[0].code);
      BOOST_REQUIRE_EQUAL(name("inita"_n), result.rows[0].scope);
      BOOST_REQUIRE_EQUAL(name("accounts"_n), result.rows[0].table);
      BOOST_REQUIRE_EQUAL(name("vexcore"_n), result.rows[0].payer);
...
BOOST_AUTO_TEST_CASE_TEMPLATE( get_table_test, TESTER_T, backing_store_ts) { try {
   ...
   // issue
   for (account_name a: accs) {
      issue_tokens( t, config::system_account_name, a, eosio::chain::asset::from_string("10000.0000 VEX") );
   }
   t.produce_blocks(1);
   ...
   if (result.rows.size() >= 4) {
      ...
      BOOST_REQUIRE_EQUAL("10000.0000 VEX", result.rows[3]["balance"].as_string());
   }
   ...
   if (result.rows.size() >= 4) {
      ...
      BOOST_REQUIRE_EQUAL("10000.0000 VEX", result.rows[0]["balance"].as_string());
   }
   ...
   if (result.rows.size() >= 4) {
      ...
      BOOST_REQUIRE_EQUAL("10000.0000 VEX", result.rows[0]["data"]["balance"].as_string());
      BOOST_REQUIRE_EQUAL("vexcore", result.rows[0]["payer"].as_string());
      BOOST_REQUIRE_EQUAL("vexcore", result.rows[1]["payer"].as_string());
      BOOST_REQUIRE_EQUAL("vexcore", result.rows[2]["payer"].as_string());
      BOOST_REQUIRE_EQUAL("vexcore", result.rows[3]["payer"].as_string());
   }
   p.show_payer = false;
...
BOOST_AUTO_TEST_CASE_TEMPLATE( get_table_by_seckey_test, TESTER_T, backing_store_ts) { try {
   ...
   auto bidname = [&t]( const account_name& bidder, const account_name& newname, const asset& bid ) {
      return t.push_action( "vexcore"_n, "bidname"_n, bidder, fc::mutable_variant_object()
                           ("bidder",  bidder)
                           ("newname", newname)
                           ("bid", bid)
                            );
   ...
   bidname("inita"_n, "com"_n, eosio::chain::asset::from_string("10.0000 VEX"));
   bidname("initb"_n, "org"_n, eosio::chain::asset::from_string("11.0000 VEX"));
   bidname("initc"_n, "io"_n, eosio::chain::asset::from_string("12.0000 VEX"));
   bidname("initd"_n, "html"_n, eosio::chain::asset::from_string("14.0000 VEX"));
   t.produce_blocks(1);
   ...
   bidname("inita"_n, name(all_digits_name_1), eosio::chain::asset::from_string("1.0000 VEX"));
   bidname("inita"_n, name(all_digits_name_2), eosio::chain::asset::from_string("1.0000 VEX"));
   t.produce_blocks(1);
