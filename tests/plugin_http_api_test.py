...
class PluginHttpTest(unittest.TestCase):
    ...
    def startEnv(self) :
        ...
        start_nodeos_cmd = ("%s -e -p vexcore %s %s ") % (Utils.EosServerPath, nodeos_plugins, nodeos_flags)
    ...
    def test_ChainApi(self) :
        ...
        valid_cmd = default_cmd + self.http_post_str + ("'{\"code\":\"vex.token\", \"account\":\"unknown\"}'")
        ret_json = Utils.runCmdReturnJson(valid_cmd)
        self.assertEqual(ret_json["code"], 500)
        ...
        valid_cmd = default_cmd + self.http_post_str + ("'{\"code\":\"vex.token\",\"symbol\":\"VEX\"}'")
        ret_json = Utils.runCmdReturnJson(valid_cmd)
        self.assertEqual(ret_json["code"], 500)
        ...
        valid_cmd = ("%s%s '{%s,%s,%s}'") % (default_cmd,
                                             ...
                                             "\"code\":\"vex.token\"",
                                             "\"action\":\"issue\"",
                                             "\"args\":{\"to\":\"vex.token\", \"quantity\":\"1.0000\%20EOS\",\"memo\":\"m\"}")
        ret_json = Utils.runCmdReturnJson(valid_cmd)
        self.assertEqual(ret_json["code"], 500)
        ...
        valid_cmd = ("%s%s '{%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s}'") % \
                    (default_cmd,
                     ...
                     "\"actions\":[{\"account\":\"vex.token\",\"name\": \"transfer\",\"authorization\": [{\"actor\": \"han\",\"permission\": \"active\"}],\"data\": \"000000000000a6690000000000ea305501000000000000000453595300000000016d\"}]",
