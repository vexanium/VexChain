...
class Cluster(object):
    ...
    def launch(self, pnodes=1, unstartedNodes=0, totalNodes=1, prodCount=1, topo="mesh", delay=1, onlyBios=False, dontBootstrap=False,
        ...
        self.eosioAccount=self.defProducerAccounts["vexcore"]
    ...
    def spreadFundsAndValidate(self, transferAmount=1):
        ...
        receiving transferAmount*n VEX and forwarding x-transferAmount funds. Transfer actions are spread round-robin across the cluster to vaidate system cohesiveness."""
    ...
    def parseClusterKeys(totalNodes):
        ...
        if producerKeys is None:
            Utils.Print("ERROR: Failed to parse vexcore private keys from cluster config files.")
            return None
    ...
    def bios_bootstrap(self, biosNode, totalNodes, pfSetupPolicy, silent=False):
        ...
        for name, keys in producerKeys.items():
            data="{\"from\":\"eosio\",\"to\":\"%s\",\"quantity\":\"%s\",\"memo\":\"%s\"}" % (name, initialFunds, "init vexcore transfer")
            opts="--permission eosio@active"
            if name != "vexcore":
                trans=biosNode.pushMessage(contract, action, data, opts)
                if trans is None or not trans[0]:
                    Utils.Print("ERROR: Failed to transfer funds from vex.token to %s." % (name))
                    return None
    ...
    def bootstrap(self, biosNode, totalNodes, prodCount, totalProducers, pfSetupPolicy, onlyBios=False, onlySetProds=False, loadSystemContract=True, manualProducerNodeConf=[]):
        """Create 'prodCount' init accounts and deposits 10000000000 VEX in each. If prodCount is -1 will initialize all possible producers.
        ...
        if not onlyBios:
            if prodCount == -1:
                ...
                with open(setProdsFile, "r") as f:
                    ...
                    opts="--permission eosio@active"
                    myTrans=biosNode.pushMessage("vexcore", "setprods", setProdsStr, opts)
                ...
                trans=biosNode.pushMessage("vexcore", "setprods", setProdsStr, opts)
                if trans is None or not trans[0]:
            ...
            lam = lambda: biosNode.getInfo(exitOnError=True)["head_block_producer"] != "vexcore"
        ...
        eosioTokenAccount.name="vex.token"
        ...
        eosioRamfeeAccount.name="vex.ramfee"
        ...
        eosioStakeAccount.name="vex.stake"
        ...
        trans=biosNode.pushMessage(contract, action, data, opts)
        if trans is None or not trans[0]:
            Utils.Print("ERROR: Failed to push create action to vexcore contract.")
            return None
        ...
        trans=biosNode.pushMessage(contract, action, data, opts)
        if trans is None or not trans[0]:
            Utils.Print("ERROR: Failed to push issue action to vexcore contract.")
            return None
        ...
        Utils.Print("Verify vexcore issue, Expected: %s" % (expectedAmount))
