        ...
        for fromIndex in range(namedAccounts.numAccounts):
            ...
            try:
                ...
                if trans is None or not trans[0]:
                    timeOutCount+=1
                    if timeOutCount>=3:
                        Print("Failed to push create action to vexcore contract for %d consecutive times, looks like nodeos already exited." % (timeOutCount))
                        keepProcessing=False
                        break

                    Print("Failed to push create action to vexcore contract. sleep for 5 seconds")
    ...
    while keepProcessing and count < 40:
        ...
        for fromIndexOffset in range(namedAccounts.numAccounts):
            ...
            try:
                trans=nodes[count % numNodes].pushMessage(contract, action, data, opts)
                if trans is None or not trans[0]:
                    Print("Failed to push create action to vexcore contract. sleep for 60 seconds")
                    time.sleep(60)
                time.sleep(1)
            except TypeError as ex:
