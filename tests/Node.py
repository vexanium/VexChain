...
class Node(object):
    ...
    def currencyStrToInt(balanceStr):
        """Converts currency string of form "12.3456 VEX" to int 123456"""
    ...
    def currencyIntToStr(balance, symbol):
        """Converts currency int of form 123456 to string "12.3456 VEX" where VEX is symbol string"""
    ...
    def validateFunds(self, initialBalances, transferAmount, source, accounts):
        """Validate each account has the expected VEX balance. Validate cumulative balance matches expectedTotal."""
        assert(source)
        assert(isinstance(source, Account))
        assert(accounts)
        assert(isinstance(accounts, list))
    ...
    def getAccountEosBalanceStr(self, scope):
        """Returns VEX currency0000 account balance from cleos get table command. Returned balance is string following syntax "98.0311 VEX". """
        assert isinstance(scope, str)
        amount=self.getTableAccountBalance("vex.token", scope)
        if Utils.Debug: Utils.Print("getNodeAccountEosBalance %s %s" % (scope, amount))
        assert isinstance(amount, str)
        return amount

    def getAccountEosBalance(self, scope):
        """Returns VEX currency0000 account balance from cleos get table command. Returned balance is an integer e.g. 980311. """
    ...
    def preactivateProtocolFeatures(self, featureDigests:list):
        for digest in featureDigests:
            ...
            trans=self.pushMessage("vexcore", "activate", data, opts)
