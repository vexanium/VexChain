...
issue_tokens( TESTER& t, account_name issuer, account_name to, const asset& amount,
              std::string memo = "", account_name token_contract = "vexcore"_n )
{
...
BOOST_AUTO_TEST_CASE_TEMPLATE( get_table_next_key_reverse_test, TESTER_T, backing_store_ts) { try {
   ...
   t.set_code( "vexcore"_n, contracts::eosio_token_wasm() );
   t.set_abi( "vexcore"_n, contracts::eosio_token_abi().data() );
   t.produce_blocks(1);
   ...
   t.push_action("vexcore"_n, "create"_n, "vexcore"_n, act );
   ...
   eosio::chain_apis::read_only::get_table_by_scope_params param{"vexcore"_n, "accounts"_n, "inita", "", 10};
   ...
   BOOST_REQUIRE_EQUAL(name("vexcore"_n), result.rows[4].scope);
