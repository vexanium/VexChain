...
// Handle vexcoreversion differences
namespace {
template <typename T>
...
struct intrinsic_context {
   ...
   intrinsic_context(eosio::chain::controller& control) : control{ control } {
      ...
      strx.actions.back().account = eosio::chain::name{ "vex.null" };
      strx.actions.back().authorization.push_back({ eosio::chain::name{ "vexcore" }, eosio::chain::name{ "active" } });
