...
function usage() {
  ...
  -i DIR      Directory where vexanium is installed)
...
if [[ $NONINTERACTIVE == false ]] && $FULL; then
   while true; do
      read -p "By specifying -f, removal of the vexanium data directory will require a resync of data which can take days. Do you wish to proceed? (y/n) " PROCEED
      case $PROCEED in
...
if [[ ! -d "${EOSIO_INSTALL_DIR}" ]]; then
   echo "[VEXANIUM installation ${COLOR_YELLOW}NOT${COLOR_NC} found in ${EOSIO_INSTALL_DIR}]"
else
   # As of 1.8.0, we're using a versioned directories under home: https://github.com/VEXANIUM/vexchain2.1/issues/6940
   echo "[VEXANIUM installation found: ${EOSIO_INSTALL_DIR}]" && INSTALL_PATHS+=("${EOSIO_INSTALL_DIR}") # EOSIO_INSTALL_DIR set in .environment
   while true; do
      ...
      case $PROCEED in
         ...
         0 | true | [Yy]* )
            ...
            echo " - VEXANIUM Removal Complete"
            break;;
         1 | false | [Nn]* ) echo " - Cancelled VEXANIUM Removal!"; exit 1;;
         * ) echo "Please type 'y' for yes or 'n' for no.";;
      esac
   done
fi

echo "[Removing VEXANIUM Dependencies]"
if [[ $ARCH == "Darwin" ]]; then
