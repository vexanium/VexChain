...
case "${OS_NAME}" in
	"Linux Mint")
		if [ "${OS_MAJ}" -lt 18 ]; then
			printf "You must be running Linux Mint 18.x or higher to install VEXANIUM.\\n"
			printf "Exiting now.\\n"
			exit 1
		fi
	;;
	"Ubuntu")
		if [ "${OS_MAJ}" -lt 16 ]; then
			printf "You must be running Ubuntu 16.04.x or higher to install VEXANIUM.\\n"
			printf "Exiting now.\\n"
			exit 1
		fi
	...
	"Debian")
		if [ $OS_MAJ -lt 10 ]; then
			printf "You must be running Debian 10 to install VEXANIUM, and resolve missing dependencies from unstable (sid).\n"
			printf "Exiting now.\n"
			exit 1
	fi
	;;
