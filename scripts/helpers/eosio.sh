...
if [[ -z "${NAME}" ]]; then
    ...
    else echo " ${COLOR_RED}- VEXANIUM is not supported for your Architecture!${COLOR_NC}" && exit 1
    fi
fi

# Setup yum and apt variables
if [[ $NAME =~ "Amazon Linux" ]] || [[ $NAME == "CentOS Linux" ]]; then
    if ! YUM=$( command -v yum 2>/dev/null ); then echo "${COLOR_RED}YUM must be installed to compile VEXANIUM${COLOR_NC}" && exit 1; fi
elif [[ $NAME == "Ubuntu" ]]; then
    if ! APTGET=$( command -v apt-get 2>/dev/null ); then echo "${COLOR_RED}APT-GET must be installed to compile VEXANIUM${COLOR_NC}" && exit 1; fi
fi
...
function ensure-which() {
  if ! which ls &>/dev/null; then
    while true; do
      [[ $NONINTERACTIVE == false ]] && printf "${COLOR_YELLOW}VEXANIUM compiler checks require the 'which' package: Would you like for us to install it? (y/n)?${COLOR_NC}" && read -p " " PROCEED
      echo ""
      case $PROCEED in
...
function install-directory-prompt() {
    if [[ -z $INSTALL_LOCATION ]]; then
        echo "No installation location was specified. Please provide the location where VEXANIUM is installed."
        while true; do
        ...
        # Support relative paths : https://github.com/VEXANIUM/vexchain2.1/issues/7560
        [[ ! $INSTALL_LOCATION =~ ^\/ ]] && export INSTALL_LOCATION="${CURRENT_WORKING_DIR}/$INSTALL_LOCATION"
        export EOSIO_INSTALL_DIR="$INSTALL_LOCATION"
    fi
    . ./scripts/.build_vars
    echo "VEXANIUM will be installed to: ${EOSIO_INSTALL_DIR}"
}

function previous-install-prompt() {
  if [[ -d $EOSIO_INSTALL_DIR ]]; then
    echo "VEXANIUM has already been installed into ${EOSIO_INSTALL_DIR}... It's suggested that you eosio_uninstall.sh before re-running this script."
    while true; do
...
function resources() {
    echo "${COLOR_CYAN}VEXANIUM website:${COLOR_NC} https://vexanium.com"
    echo "${COLOR_CYAN}VEXANIUM Telegram channel:${COLOR_NC} https://t.me/vexaniumcom"
    echo "${COLOR_CYAN}VEXANIUM resources:${COLOR_NC} https://eos.io/resources/"
    echo "${COLOR_CYAN}VEXANIUM Stack Exchange:${COLOR_NC} https://eosio.stackexchange.com"
}

function print_supported_linux_distros_and_exit() {
   echo "On Linux the VEXANIUM build script only supports Amazon, Centos, and Ubuntu."
...
function ensure-compiler() {
    ...
    if $PIN_COMPILER || [[ -f $CLANG_ROOT/bin/clang++ ]]; then
        ...
        if [[ $CXX =~ "clang" ]] || [[ $COMPILER_TYPE =~ "clang" ]]; then
            ...
            if [[ $NO_CPP17 == false ]]; then # https://github.com/VEXANIUM/vexchain2.1/issues/7402
                while true; do
                    echo "${COLOR_YELLOW}WARNING: Your GCC compiler ($CXX) is less performant than clang (https://github.com/VEXANIUM/vexchain2.1/issues/7402). We suggest running the build script with -P or install your own clang and try again.${COLOR_NC}"
                    [[ $NONINTERACTIVE == false ]] && printf "${COLOR_YELLOW}Do you wish to proceed anyway? (y/n)?${COLOR_NC}" && read -p " " PROCEED
                    case $PROCEED in
                        "" ) echo "What would you like to do?";;
                        0 | true | [Yy]* ) break;;
